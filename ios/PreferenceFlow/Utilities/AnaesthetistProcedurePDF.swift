//
//  AnaesthetistProcedurePDF.swift
//  PreferenceFlow
//
...
/// Renders one anaesthetist operation card (e.g. "CABG", "Craniotomy",
/// "C-section") into a single, laminate-ready A4 page — the anaesthetic
/// parallel of the surgeon operation card. Header carries the operation and
/// consultant, a highlighted notes band surfaces the special notes, then a
/// two-column body (Monitoring · IV Access · Timeline on the left, Airway &
/// Lines · Infusions · Equipment on the right) and the standard verification
/// and reference-only footer. Pure layout; mirrors the on-screen operation tab.
@MainActor
enum AnaesthetistProcedurePDF {
    // A4 at 72 dpi.
    ...
    static func data(procedure: ProcedureTemplate, doctor: Doctor, hospital: Hospital?) -> Data {
        ...
        return renderer.pdfData { ctx in
            ...
            if !procedure.specialNotes.isBlank {
                y = drawNotesBand(procedure.specialNotes, top: y) + 12
            }
    ...
    static func writeFile(procedure: ProcedureTemplate, doctor: Doctor, hospital: Hospital?) throws -> URL {
        let pdf = data(procedure: procedure, doctor: doctor, hospital: hospital)
        let procedurePart = procedure.displayName.replacingOccurrences(of: " ", with: "_")
            .replacingOccurrences(of: "/", with: "-")
        let namePart = doctor.fullName.isEmpty
            ? "Consultant"
            : doctor.fullName.replacingOccurrences(of: " ", with: "_")
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("\(namePart)_\(procedurePart)_Card.pdf")
    ...
    private static func drawHeader(procedure: ProcedureTemplate, doctor: Doctor, hospital: Hospital?) -> CGFloat {
        ...
        if !procedure.typicalStartTime.isBlank { metaParts.append("Typical start \(procedure.typicalStartTime)") }
        if !procedure.typicalLocation.isBlank { metaParts.append(procedure.typicalLocation) }
        let formatter = DateFormatter()
        formatter.dateStyle = .medium
        metaParts.append("Updated \(formatter.string(from: doctor.updatedAt))")
        metaParts.joined(separator: "  ·  ").draw(
    ...
    // MARK: - Special notes band

    /// The consultant's special notes for this operation, highlighted like the
    /// on-screen callout.
    ...
    private static func drawColumns(procedure: ProcedureTemplate, top: CGFloat) -> CGFloat {
        let colWidth = (pageSize.width - margin * 2 - gutter) / 2
        let leftX = margin
        let rightX = margin + colWidth + gutter

        var leftY = top
        leftY = drawCard(title: "Monitoring", icon: "waveform.path.ecg",
                         items: procedure.monitoring.map { $0.rawValue }.sorted().map { CardItem(text: $0) },
                         x: leftX, y: leftY, width: colWidth)
        leftY = drawCard(title: "IV Access", icon: "syringe",
                         items: ivItems(procedure), x: leftX, y: leftY + 10, width: colWidth)
        if !procedure.timeline.isEmpty {
            leftY = drawCard(title: "Preparation Timeline", icon: "clock",
                             items: procedure.timeline.map { entry in
                                 CardItem(text: entry.time.isBlank ? entry.event : "\(entry.time) — \(entry.event)")
                             },
                             x: leftX, y: leftY + 10, width: colWidth)
        }

        var rightY = top
        rightY = drawCard(title: "Airway & Lines", icon: "lungs.fill",
                          items: airwayLineItems(procedure), x: rightX, y: rightY, width: colWidth)
        rightY = drawCard(title: "Infusions", icon: "ivfluid.bag",
                          items: procedure.infusions.isBlank ? [] : [CardItem(text: procedure.infusions)],
                          x: rightX, y: rightY + 10, width: colWidth)
        rightY = drawCard(title: "Equipment Checklist", icon: "checklist",
                          items: procedure.equipmentChecklist
                              .filter { !$0.text.isBlank }
                              .map { CardItem(text: $0.text) },
                          x: rightX, y: rightY + 10, width: colWidth)

        return max(leftY, rightY)
    }

    private static func ivItems(_ procedure: ProcedureTemplate) -> [CardItem] {
        var items: [CardItem] = []
        if !procedure.ivCount.isBlank { items.append(CardItem(text: "Number: \(procedure.ivCount)")) }
        if !procedure.ivSize.isBlank { items.append(CardItem(text: "Size: \(procedure.ivSize)")) }
        if !procedure.ivLocation.isBlank { items.append(CardItem(text: "Location: \(procedure.ivLocation)")) }
        return items
    }

    private static func airwayLineItems(_ procedure: ProcedureTemplate) -> [CardItem] {
        var items: [CardItem] = []
        if !procedure.airwayNotes.isBlank { items.append(CardItem(text: procedure.airwayNotes, subtext: "Airway")) }
        if !procedure.lineSetup.isBlank { items.append(CardItem(text: procedure.lineSetup, subtext: "Line setup")) }
        return items
    }
    ...
    private static func drawCard(title: String, icon: String, items: [CardItem], x: CGFloat, y: CGFloat, width: CGFloat) -> CGFloat {
        ...
        let displayItems: [CardItem] = isEmpty ? [CardItem(text: "Not recorded — see app")] : items
